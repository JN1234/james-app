    ...
    public List<String> createUser(User user){
        ...
        try {
            if (!userRepository.existsByEmail(user.getEmail())){
                if(user.getPassword().isEmpty() || user.getPassword().length() < 7){

                    return List.of("Password be at least 4 characters long.");
                }
                ...
                int counter=0;
                for(int a=0; a < passString.length;a++){
                    int asciiVal = Character.codePointAt(passString[a],0);
                ...
                if(counter > 0){
                    ...
                    return List.of("Password must consist of numbers 1 - 9 only");
                }
                else {
